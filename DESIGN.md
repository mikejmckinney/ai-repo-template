...
>
> **Template repo note (`ai-repo-template`):** This repo has no product UI. The sections below are a **generic contract template** shipped so derived projects and agents know the expected shape. During `template-seed` onboarding, use the OpenCode `repo-onboarding` skill to preserve useful guidance and add verified product-specific direction **before frontend implementation starts**.
