...
4. **[`.github/PLAN_TEMPLATE.md`](PLAN_TEMPLATE.md)** — copy this template into a comment on any issue you're about to implement, before writing code. See ADR-011 and the template's `plan_compliance` block for the full rules and exemptions.
...
[`docs/guides/subagent-bootstrap-reference.md`](../docs/guides/subagent-bootstrap-reference.md) and
[`docs/compliance_schemas.md`](../docs/compliance_schemas.md). Do not claim CI proves runtime
...
(ADR-031 monolithic default). Read it before deciding to absorb role-owned work.

**Why only Copilot's overlay carries this clarification.** Other platform
overlays (`.cursor/`, `.gemini/`, `CLAUDE.md`) pick up OP guidance via
`AGENTS.md` → `.github/prompts/op-issue-workflow.md` and need no parallel update.
...
- [`.context/rules/agent_ownership.md`](../.context/rules/agent_ownership.md) — path-ownership map (ADR-009 parallelism tooling; not a startup-required rule per ADR-031).
- [`.github/prompts/op-issue-workflow.md`](prompts/op-issue-workflow.md) — OP issue→merge playbook and monolithic vs dispatch defaults.
