...
6. PR body's `## Plan` section should link the plan comment (and any
   revisions) and summarize the latest version in one line. Refresh it
   on every "Plan revision" comment. Judge advises (REQUEST_CHANGES,
   not BLOCK) at diff-gate when missing or stale.
