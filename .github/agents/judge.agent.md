...
11. **Issue / parent-PR link** (per ADR-011): the PR body must reference an issue or parent PR (`Closes #NN`, `Refs #NN`, `Implements ADR-NNN`, etc.). BLOCK if the body contains no `#NN` reference AND the PR carries no exemption label (`chore:no-plan`, `smoke-test`) AND the author is not a known automation bot (Renovate, Dependabot). The required-text scaffold in `.github/pull_request_template.md` makes this easy to spot — an empty `**Closes / Refs:** #` line is a BLOCK.
12. **Plan-as-comment** (per ADR-011, advisory in v1): if the linked issue lacks an Implementation Plan comment matching `.github/PLAN_TEMPLATE.md` (heuristic: presence of `## 📋 Implementation Plan` heading) and the issue carries no `chore:no-plan` label, REQUEST_CHANGES with a pointer to the template. Do not BLOCK on this in v1 — the gate is review-time advisory until #155 promotes it.
