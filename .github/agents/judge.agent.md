...
- [ ] **An Analyst Pre-Flight Report is posted on the issue with verdict PASS when *either* of these trigger conditions holds: (a) the issue references a numbered project prompt file matching `.github/prompts/NN-*.md` (where `NN` is a two-digit prefix, e.g., `01-init.md`) and the prompt describes an interactive/operational deliverable, *or* (b) the issue carries the `gate:preflight-required` label.** Do not apply this gate to shared procedural prompts (`pr-resolve-all.md`, `repo-onboarding.md`, `copilot-onboarding.md`, `expand-backlog-entry.md`) or prompt documentation (`README.md`) under `.github/prompts/` — those exemptions hold even if the issue carries the `gate:preflight-required` label because they describe procedures, not deliverables. BLOCK if the report is missing, or if the report exists with verdict FAIL or HOLD. The Pre-Flight Report validates the work's user outcome against the 15-minute test (see `analyst.agent.md` → "Pre-Flight Validation"). ADR-005 introduced this gate for prompt files; ADR-014 extends it to label-triggered issues so ad-hoc feature work and ADR proposals introducing new agent surfaces are also caught.
