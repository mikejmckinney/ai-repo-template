...
13. **Plan-revision sync** (advisory in v1): if the linked issue has a post-PR-open "Plan revision" comment, the PR body should include a short `Revision history` note with a link to that issue comment. If missing, REQUEST_CHANGES with explicit guidance to sync PR context before re-running reviews. Do not BLOCK on this in v1.
