...
- Don't disable or `.skip` tests without filing a follow-up GitHub issue and labeling it `agent:blocked` or similar (per ADR-025; the legacy `coordination.md` lock board is no longer the primary tracker).
