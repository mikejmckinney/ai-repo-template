...
if ! opencode models openrouter | grep -qx 'openrouter/moonshotai/kimi-k3'; then
  printf 'OpenRouter provider does not expose openrouter/moonshotai/kimi-k3\n' >&2
  exit 1
fi
...
jq -n '{status: "success", kimi_model: "openrouter/moonshotai/kimi-k3", sol_model: "openai/gpt-5.6-sol", grok_model: "cursor-grok-4.5-medium"}'
