...
20. **Sandbox dogfood evidence (ADR-029)**: for PRs that modify agent contracts, gate predicates, compliance schemas, or `.agents/.context/.github/agents` loaders, the PR body must include a `## Sandbox dogfood evidence` section with exactly two labels — `Sandbox issue:` and `Sandbox PR:` — each pointing to a real sandbox URL (issue link and PR link respectively, in a sandbox sibling repo of your choice; the example used by this repo is `mikejmckinney/ai-repo-template-sandbox` but ADR-029 does not pin the sandbox repo URL). REQUEST_CHANGES when the section is absent, either label is missing, or either URL is empty/malformed. BLOCK when the section is filled with plausible-looking URLs that do not actually exercise the change (the security boundary is human review — exercise it; sandbox-repo write-access controls + your judgement are the only enforcement for content). Per ADR-029 §1 there is no override field, no bypass token, and no "n/a" textual escape: if a change cannot be exercised through a sandbox, the change is out of scope for the default branch and the PR should be split or descoped, not exempted. The `scripts/checks/157-sandbox-evidence-labels.sh` advisory check provides structural drift detection but does not validate URL content — that is your job.
