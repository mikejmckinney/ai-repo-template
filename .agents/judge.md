...
20. **Sandbox dogfood evidence (ADR-029)**: for PRs that modify agent contracts, gate predicates, compliance schemas, or `.agents/.context/.github/agents` loaders, the PR body must include a `## Sandbox dogfood evidence` section with exactly five labels — `Sandbox issue:`, `Sandbox PR:`, `Negative control:`, `Positive control:`, `Role-comment render:` — each pointing to a real sandbox URL that satisfies the ADR-029 §3 per-field regex AND content check. URLs must be pairwise-distinct. REQUEST_CHANGES when the section is absent or any label fails regex/content. BLOCK when the section is filled with plausible-looking URLs that resolve to non-existent comments or unrelated resources (the §5 security boundary is human review — exercise it). Per ADR-029 §1 there is no override field, no bypass token, and no "n/a" textual escape: if a change cannot be exercised through the sandbox, the change is out of scope for this repo's default branch and the PR should be split or descoped, not exempted. The `scripts/checks/157-sandbox-evidence-labels.sh` advisory check provides drift detection but does not validate URL content — that is your job.
