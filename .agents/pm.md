...
2. Read each role's `owned_paths:` frontmatter in [`.agents/`](../.agents/) and [`docs/guides/multi-agent-coordination.md`](../docs/guides/multi-agent-coordination.md) § "Conflict-Avoidance Hierarchy".
...
- Assign each task to a single role based on canonical `owned_paths:` frontmatter in [`.agents/`](../.agents/).
...
- Don't edit files outside `.context/state/**` without a documented PM coordination reason.
