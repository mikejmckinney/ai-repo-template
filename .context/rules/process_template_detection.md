...
- Otherwise:
  ...
  - During **Mode B** onboarding, also reset `.context/00_INDEX.md`, `.context/roadmap.md`, `.context/vision/README.md`, and root `DESIGN.md` from the **canonical stub** blocks in `.github/prompts/repo-onboarding-stubs.md` (procedure: `.github/prompts/repo-onboarding.md` Track B), even if the live template repo versions of those files no longer contain `TEMPLATE_PLACEHOLDER`.
  - After that reset, repopulate those four files with project-specific content — including product-specific design direction in `DESIGN.md` — before continuing past Phase 0 or regenerating AI_REPO_GUIDE.md.
