...
- Otherwise:
  ...
  - **Clear the template's working state (Mode B only).** `.context/state/*.md` and `.context/sessions/*.md` ship populated with ai-repo-template's own task data and session history. Downstream projects must reset these to empty templates during onboarding so re-read cadence (see [`process_session_state.md`](process_session_state.md)) doesn't reinforce the template's stale state as if it were the project's reality. Specifically:
    - `.context/state/_active.md` — clear all `## Task: <branch>` sections under the `# Active Tasks` header (the body should contain only the file header until the first real task is claimed); keep the schema comment and the `TEMPLATE_PLACEHOLDER` marker. New normal-work branches should record live state in the latest `agent-state:v1` GitHub comment per ADR-025; this file is preserved only as a legacy-compatibility view. (Multi-task schema: see ADR-018.)
    - `.context/sessions/latest_summary.md` — replace body with a single "No sessions yet" line (keep the marker until first close-out).
    - `.context/state/coordination.md` — clear all entries under `## Active Locks`, `## Recent History`, `## Blocked / Waiting`, and `## PM Notes`; keep the section headers, `## Lock Template`, the ADR-025 compatibility note, and `TEMPLATE_PLACEHOLDER` marker.
    - Do NOT delete `agent_state_comment_template.md` or `README.md` files in those directories — `agent_state_comment_template.md` is the canonical template downstream agents copy into GitHub issue/PR comments per ADR-025.
