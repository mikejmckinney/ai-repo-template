...
- **Critic**: cite anti-pattern IDs in your review notes when flagging drift (e.g., "AP1 trigger: process_session_state.md now covers three concerns"). Use `MAJOR CONCERNS` for block-able anti-patterns (`AP1`, `AP2`, `AP3`, `AP6`, `AP7`) and `CRAFT NOTES` for advisory ones (`AP4`, `AP5`, `AP8`) — match the citation severity to the anti-pattern's diff-gate designation.
...
**Currently triggered by**: `AGENTS.md` (335 lines, version 9, 18+ concerns). Tracked in sub-issue 2 of parent epic #251.
