...
Each role is pinned to a cost tier (High / Mid / Low) in its platform registration overlays — `.claude/agents/<role>.md`, `.github/agents/<role>.agent.md`, `.cursor/agents/<role>.md`, and `.codex/agents/<role>.toml` — per [ADR-019](../../docs/decisions/adr-019-per-role-model-tiering.md). The canonical role body lives in `.agents/<role>.md` (platform-agnostic, ADR-023) and contains no `model:` field; tier values are platform-specific frontmatter and stay in the overlays. The canonical tier table is in ADR-019; do not duplicate it here. Five behavioral notes that affect day-to-day agent work:

1. **Per-platform value-space divergence is intentional.** `.claude/agents/*.md` uses Anthropic-only model strings (`claude-opus-4-7`, `claude-sonnet-4-6`, `inherit`). `.github/agents/*.agent.md` uses Copilot's qualified format (`'Claude Opus 4.7 (copilot)'`, etc.) or omits the field for Low-tier roles. `.cursor/agents/*.md` uses YAML frontmatter with `model:`, `readonly:`, and `is_background:`; this repo currently pins `Claude Opus 4.8`, `Claude 4.6 Sonnet`, `inherit`, and `fast` as the allowed Cursor value-space. `.codex/agents/*.toml` uses Codex custom-agent TOML with required `name`, `description`, and `developer_instructions`, and this repo additionally pins `model`, `model_reasoning_effort`, and `sandbox_mode` per role. `test.sh` enforces per-platform allowlists in `scripts/checks/050-agent-mirror.sh`; description-parity (byte-identical `description:` field across canonical + overlays) is unchanged.
...
3. **Cursor inherits the parent tier unless you pin the overlay.** `model: inherit` on Cursor behaves like the parent session's tier ceiling, so High-tier descendants still depend on the parent model being high enough. Keep OP at top tier when a workflow needs high-tier Cursor descendants, and treat nested-subagent MAX-mode inheritance drift as a runtime caveat rather than a guarantee of deeper pins.
4. **Codex custom agents inherit omitted settings from the parent session.** Codex allows per-agent overrides for `model`, `model_reasoning_effort`, and `sandbox_mode`, but omitted fields inherit the parent session. This repo uses explicit pins for all first-class Codex overlays and keeps `agents.max_depth` conservative; unlike Copilot, no published hard cost-tier ceiling is documented today.
5. **Per-task tier upshift via the plan template.** Use the `Model tier:` field in [`.github/PLAN_TEMPLATE.md`](../../.github/PLAN_TEMPLATE.md) to upshift a specific task above its role's default tier. Default is `mid`; when set to `top`, include a one-line justification. Antigravity remains documentation-only until a public custom-agent file format exists.
