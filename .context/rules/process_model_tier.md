...
2. If the subagent must touch the working tree directly, require a `git log origin/<branch> -1 --oneline` verification snippet in `subagent_compliance.verification[].evidence` so the parent OP catches the drift in the same response rather than after-the-fact.
