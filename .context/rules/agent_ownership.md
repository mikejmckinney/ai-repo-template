...
|------------|----------------------------------------------------------------------|-------------------------------|
| ZZAnalyst  | `docs/research/**`                                                   | nothing (research-only)       |
| ZZArchitect| `AGENTS.md`, `docs/decisions/**`, `.context/roadmap.md`, `.context/vision/architecture/**`, `.context/rules/**` (except `agent_ownership.md`) | nothing (plan-only) |
| ZZFrontend | `src/frontend/**`, `src/components/**`, `src/pages/**`, `src/styles/**`, `public/**`, colocated `*.test.*` / `*.spec.*` under those paths | UI-adjacent tests in `tests/ui/**` |
| ZZBackend  | `src/backend/**`, `src/api/**`, `src/server/**`, `src/models/**`, `migrations/**`, `db/**`, colocated `*.test.*` / `*.spec.*` under those paths | API-adjacent tests in `tests/api/**` |
| ZZPM       | `.context/state/**`, `.context/rules/agent_ownership.md`            | nothing (dispatch-only)       |
| ZZQA       | `tests/**`, `e2e/**`                                                 | nothing                       |
| ZZDevOps   | .github/workflows/**, config/**, install.sh, test.sh, scripts/**, .pre-commit-config.yaml.template, .cursorignore | nothing                       |
| ZZDocs     | README.md, AI_REPO_GUIDE.md, CLAUDE.md, AGENT.md, docs/** (except docs/decisions/**, docs/research/**)  | nothing                       |
| ZZJudge    | nothing (review-only, `.github/agents/judge.agent.md`)               | nothing                       |
| ZZCritic   | nothing (review-only, `.github/agents/critic.agent.md`)              | nothing                       |
