...
| Edit the legacy schema of `.context/state/coordination.md` `## Lock Template` or `.context/state/_active.md` | The other legacy schema surface, `.context/state/README.md`, `.github/workflows/agent-coordination-sync.yml` if it still emits compatibility suggestions, and an ADR note explaining why the legacy view still needs schema evolution after ADR-025 | These files are transitional compatibility views. If they must change, keep the remaining compatibility tooling aligned and explain why the change was not made in the GitHub-first `agent-state:v1` schema instead |
| Change the exact-output exemption attestation header contract in `.agents/judge.md` §"Exemption Attestation" (the `## Judge — DECISION` two-line shape) | `.cursor/BUGBOT.md` (skip-list note), `.gemini/styleguide.md` (skip-list note), `.github/copilot-instructions.md` (skip-list note), `.github/workflows/claude.yml` (review prompt) — every review surface that might otherwise flag the H2 as non-conventional must be re-aligned in lockstep, **and** ADR-028 §A2 must be amended in the same PR | The H2 is the load-bearing predicate surface per ADR-028 §A1¶3/§A2; silent drift between the canonical header and a review agent's heading-shape heuristic re-opens RC2 |
