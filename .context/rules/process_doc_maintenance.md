...
| Rename or restructure a section heading in any `.context/rules/process_*.md` file | The link table in `AGENTS.md` (verify each cited heading still resolves) **and** any cross-file references inside other `.context/rules/process_*.md` files **and** any ADR or guide that cites the section by anchor | The thin AGENTS.md links into per-concern process files by anchor (ADR-021); silent drift breaks the link table the same way the pre-decomposition pointer files broke (#207, #208) |
