...
| A postmortem under `docs/postmortems/**` whose "What generalizes" field is **Yes** | A follow-up issue, PR, ADR, or rule edit cited from the postmortem's Action Items (in the same PR or linked from it) | A postmortem alone changes nothing; the lesson only lands when a rule/prompt/ADR/issue ships |
...
| Add a pipeline label to `.github/workflows/AGENT-PIPELINE-GUIDE.md`'s label table | `scripts/setup.sh` `_ensure_label` list (and the fallback warning's manual-label list in the same script) | Labels documented but not auto-created cause silent setup drift |
