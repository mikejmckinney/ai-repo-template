...
| Rename or move any `.context/rules/process_*.md` file (or restructure a section heading inside one) | The link table in `AGENTS.md` (verify each row still resolves to the renamed/moved file) **and** the section-anchor redirect table in `AGENTS.md` (update the right-hand-side path) **and** any cross-file references inside other `.context/rules/process_*.md` files **and** any ADR or guide that cites the file or section by path/anchor | The thin AGENTS.md links into per-concern files by file path (and the redirect table maps legacy section anchors to new file paths); silent drift breaks the link table or strands the redirect entries (#207, #208) |
