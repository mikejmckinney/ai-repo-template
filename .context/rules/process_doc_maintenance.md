...
| Change cadence / format of `.context/state/_active.md` (legacy compatibility), `sessions/latest_summary.md`, or `.context/state/agent_state_comment_template.md` | `.context/state/README.md` (or `.context/sessions/README.md`) and the affected templates | These files are agent-consumed; format drift breaks parsing |
| Change the `agent-state:v1` comment schema (HTML marker, required sections, `Status:` enum, label semantics) | `.context/state/agent_state_comment_template.md` **and** `.context/rules/process_session_state.md` (cadence section) **and** `docs/decisions/adr-025-github-first-agent-state.md` (amendment) **and** `docs/guides/agent-pipeline.md` (label table) | The template, the cadence rule, the ADR, and the label table are co-load-bearing for ADR-025; drift between them produces silent state drift |
