...
## What Shipped
- **Epic #255 modularization is now complete.** Phase 4d's deferred acceptance criterion (no legacy `scripts/test-*.sh` shims) is satisfied by #280; the deferred coverage expansion from PR #278 R5 is satisfied by #281. `bash test.sh` reports **404 / 1 / 0** on `main` (was 365 / 1 / 0 at #255 close, +39 from per-bats-assertion counts surfaced by `run_bats_check` and +51 from new syntax-check files, –12 from de-dup of `install.sh`/`test.sh` that legacy double-counted). `scripts/lib/bats-helpers.sh` is now the canonical wrapper for any module that gates on a bats file — single-source guard for "bats not installed" and per-assertion count formatting.
