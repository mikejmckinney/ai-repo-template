...
- **`_active.md`** — multi-task file (ADR-018). One `## Task: <branch-name>` section per in-flight branch under the `# Active Tasks` header. Cap: **~20 lines per section** (no fixed file-level cap; bounded growth comes from disciplined cleanup at close-out).
  ...
  - **Cleanup** — remove your `## Task:` section only at **genuine task close-out** (the agent reports completion with no further actions queued). Mid-task wait-for-input pauses (clarification questions, plan-mode approvals) do **not** trigger removal — they only refresh `sessions/latest_summary.md`. The genuine-done close-out commit lands all three changes together: (i) `_active.md` `## Task:` section removed, (ii) `coordination.md` lock moved from Active Locks to Recent History with `**Result**:` line, (iii) `sessions/latest_summary.md` `Status` field set to `done`. See `AGENTS.md` §"Session-state cadence" → "Close-out (every wait-for-input pause)" for the working-log refresh vs lock release split. Forgotten sections at genuine done are caught by PM's `coordination.md` reconciliation pass.
