...
**Fix dispatch:** `run-postmerge-retro-fix.sh` — Cursor local edits or Gemini JSON `file_edits`; requires `contents: write` + `pull-requests: write` on fix job. On `auto`, the **fix pass prefers Gemini** when a key is available (JSON apply path); advisory/finalize still prefer Cursor first. After the draft fix PR is created (or found open), `update-umbrella-fix-link.sh` replaces the umbrella Meta placeholder `(pending — fix job)` with the PR URL.
