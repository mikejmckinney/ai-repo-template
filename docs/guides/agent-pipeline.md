...
- `CLAUDE_OAUTH_SECRET`: one-year Claude subscription OAuth token generated by
  `claude setup-token` and synchronized with
  `scripts/sync-claude-oauth-secret.sh`. The pre-merge workflow maps it to
  `CLAUDE_CODE_OAUTH_TOKEN`; daily and weekly workflows do not consume it.
