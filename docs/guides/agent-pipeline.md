...
**Bot-mention neutralization (relay path)**: when `agent-relay-reviews.yml` forwards review comment bodies in the relay comment, it wraps every `@`-mention except `@copilot` in backticks so forwarded text does not re-trigger other bots. The regex negative lookahead `(?!copilot(?:[^A-Za-z0-9_-]|$))` preserves the bare `@copilot` dispatch handle while wrapping all others (including `@copilot-swe-agent`). If a relayed review body re-triggers an unexpected bot, verify the neutralization step ran and check for an unguarded mention. See `agent-relay-reviews.yml` lines ~542–546 for the canonical jq implementation. (Issue #225: a relayed Codex banner re-triggered Codex into a full session against the relay comment.)
