...
**Finding classifier (#456):** LLM emits triage fields on every row in all three buckets; automation derives `priority_band` through shared `scripts/workflows/lib/finding_priority.py` (the existing `classify-finding-priority.py` path is a CLI compatibility wrapper). Deprecated `severity` is rejected in daily and weekly review.

**Evidence truncation visibility (#460):** After `collect-postmerge-evidence.sh`, `compute-evidence-coverage.py` records diff/HEAD byte totals and `would_truncate` before prompt assembly. Bounded runs emit `::warning::` when truncation is predicted; `pr_evidence_coverage[]` on `daily-retro.json` and the umbrella Meta **Evidence coverage** block surface route + `routing_context` for operators. When any PR truncates, the umbrella **Summary** shows a `[!WARNING]` callout above **Findings**; Meta keeps the detailed per-PR lines. Canonical coverage lives in `daily-retro.json`, workflow artifacts, and the umbrella JSON snapshot comment — not in the findings table. `reconstruct-daily-retro-from-umbrella.py` recovers findings only; use snapshot/artifact for coverage.

**Adaptive evidence routing (#461):** When `POSTMERGE_RETRO_ADAPTIVE_EVIDENCE=true` (default), `run-postmerge-retro.sh` routes each PR to a **single** LLM pass before findings merge: `bounded` when caps fit; `full-evidence-cursor` (Cursor SDK reads full `diff.patch` + omitted HEAD paths) when truncation is predicted and Cursor is available; `full-evidence-antigravity` when provider resolves to Gemini, `ADVISORY_ANTIGRAVITY_ENABLED=true`, and `POSTMERGE_RETRO_ANTIGRAVITY_ON_TRUNCATE=true`; otherwise `bounded-fallback` with `::warning::` (job does not fail). Monolithic Arm B and fix-job routing are out of scope. Default caps: diff `POSTMERGE_RETRO_DIFF_LIMIT` (300k bytes); HEAD 12k/file and 120k total (sequential), 8k/80k (monolithic).
...
**Fix dispatch:** `run-postmerge-retro-fix.sh` — Cursor local edits or Gemini JSON `file_edits`; requires `contents: write` + `pull-requests: write` on fix job. Re-exec uses a child-owned dir under `.artifacts/postmerge-retro/fix-reexec-<RUN_DATE>/` (no orphan parent `mktemp`). Daily and weekly adapters use `scripts/workflows/lib/run-batch-fix.sh` for workflow stripping, verification fallback, commit/no-diff handling, push, PR publication, and umbrella linkage. When a fix re-run produces no diff but an open draft PR exists, automation **skips** `gh pr edit` (umbrella link still updates). Shared `checkout-fix-branch.sh` prepares the working branch (continue open draft PR / existing remote branch + merge `main`, or fresh from `main`; fail on closed PR + lingering branch). Provider routing uses `pick-advisory-provider.sh` + `invoke-advisory-llm.sh`. Reporting agents must emit `repro_steps[]` on each `follow_up_issues` row. Fix agents record per-finding pre/post repro in `retro/fix-verify-<RUN_DATE>.json` and render `## Fix verification` on the draft PR (ADR-029 §1.1). When repo variable `FIX_JOB_SANDBOX_VERIFY=true`, the fix job may push `test/fix-retro-<RUN_DATE>` to sandbox via `sandbox-sync-fix-branch.sh` using `SANDBOX_BOOTSTRAP_TOKEN` (default off). Phase durations log as `::notice::fix-phase=…`. Shared `umbrella-lifecycle.sh` handles issue transport and reference persistence while cadence adapters preserve daily merge-index/evidence hooks and weekly snapshots.
...
2. Batch → `weekly-review.json`; automation derives the same Impact/Trigger/Cost/Guard/Band classifier used by daily retro and rejects `severity`.
3. **One umbrella issue per ISO week** (`<!-- weekly-review:YYYY-Www -->`; skip when zero findings). The hybrid body contains a triage summary table plus detailed blocks with full body, reproduction, `repo-*` key, and evidence blob links. Existing block-only umbrellas upgrade on append.
4. **Draft fix PR** `weekly/fix-YYYY-Www` when findings exist; native issue link via **`Fixes #N`**. Weekly uses a child-owned `.artifacts/weekly-review/fix-reexec-<RUN_WEEK>/` runner copy plus the shared batch-fix publication lifecycle and narrow superseded missing-path precheck; no-diff reruns preserve existing PR bodies.

**Dispatch:** `scripts/workflows/weekly-review/` — prompts `weekly-repo-review.md`, `weekly-repo-review-fix.md`; templates `weekly-review-umbrella.md`, `weekly-review-fix-pr.md`. Context via `WEEKLY_REVIEW_CONTEXT_PROFILE` (default **`full`**, pack only). Provider selection and Cursor/Gemini invocation use shared Phase A libraries; weekly Antigravity remains a cadence-specific runner. Weekly evidence coverage/adaptive context routing is deferred until explicit caps exist or truncation is observed.
...
| `POSTMERGE_RETRO_ADAPTIVE_EVIDENCE` | `true` | When true, per-PR retro routes to full-evidence Cursor/Antigravity when truncation is predicted (single pass). Set `false` to always use bounded truncated prompts. |
| `POSTMERGE_RETRO_ANTIGRAVITY_ON_TRUNCATE` | `true` | When Gemini is the resolved retro provider and truncation is predicted, use Antigravity full-diff mount when `ADVISORY_ANTIGRAVITY_ENABLED=true`. |
