...
1. **Citing a pattern is the start of a conversation, not the end of one.** A reviewer writing "this is becoming a God Object — see `CAP-AP1`" must say *which detection signals fired* and *what the code-layer remediation looks like for this case*. The pattern name is a handle; the argument is the work. Reviewers who cite pattern names without justification are doing the thing the [§"Goal Substitution"](#cap1--goal-substitution) entry warns against — substituting a deliverable (a citation) for the outcome (a clearer review).
