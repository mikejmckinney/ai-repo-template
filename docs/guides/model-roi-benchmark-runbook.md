...
### Task-parallelism Phase 0C transport apparatus

Issue #581 is a separate no-spend apparatus stacked on dependency head
`d00f9c432591e88b24b5c63eb241b0ec6ca36f13`. It does not modify or reinterpret
the completed Phase 0B A/B/prompt-gated-C campaign. `requirements.in` contains
the direct inputs; `requirements.txt` is the hashed transitive lock generated by
`uv 0.11.29` for Python 3.12. Reproduce, install, and run the no-spend journey:
...
Agent Card discovery, a server-validated `A2A-Version: 1.0` header, and structured
non-streaming JSON-RPC payload echo. `1.0.0` identifies the specification release,
not the wire identifier. The fixture reader has no GitHub or transport behavior,
and direct `Message` responses deliberately avoid task lifecycle. This evidence
does not establish production-shaped GitHub/A2A transport equivalence, task
...
planner graph and deterministic gate as separate records and freeze both before
arm order selection; no preflight fixture digest is paid graph evidence. A later
approval must name that freeze and the exact maximum: one shared planner plus
four processes per arm, nine total for the sequential pair, no more than three
simultaneous, a 45-minute end-to-end limit per arm, and no candidate-defect
replacement.
