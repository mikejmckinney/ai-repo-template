...
| `install.sh` at the repo root | Retained only for compatibility with users who selected this repository through GitHub Codespaces' legacy account-level "Dotfiles" path. New repositories use `.devcontainer/devcontainer.json`; removal requires a separate compatibility decision. |
