...
| `test.sh` at the repo root | Invoked by `.github/workflows/ci-tests.yml` as `./test.sh`, referenced from `README.md`, `AI_REPO_GUIDE.md`, `CLAUDE.md`, and the devops role files. `scripts/` is scoped to **post-clone project customization** (`setup.sh`, `verify-env.sh`) — a different role from template-level bootstrap and integrity tooling. |
