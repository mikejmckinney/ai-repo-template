...
Note: this is consistent with `pr-resolve-all.md`'s "classify-then-decide" Round discipline Rule 3 (issue #220). Rule 3 keeps substantive fixes in separate commits and reserves bundling for a final nit-only commit; this rule applies the same single-concern discipline per commit.
