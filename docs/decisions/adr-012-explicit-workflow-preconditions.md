...
We do NOT add a `.context/rules/` companion file, because the rule is a precondition for *all* work (not a domain-specific constraint), and AGENTS.md is already the surface every agent reads on onboarding. A separate file would create a second source of truth at higher drift risk.
