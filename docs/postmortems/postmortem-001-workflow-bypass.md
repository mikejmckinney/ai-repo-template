<!--
Provenance: this postmortem was originally authored in
mikejmckinney/cmmc-level2-aws-enclave-reference2 (a project bootstrapped from
this template) and is mirrored here verbatim under
docs/postmortems/postmortem-001-workflow-bypass.md because the lessons
...
append-only for facts. If new evidence appears, add a follow-up section at
the bottom rather than editing the original.

Inline links in the body below use the original repo's relative paths
(../../AGENTS.md etc.). In this template repo those resolve to the template's
