...
def find_secret(value: object) -> bool:
    if isinstance(value, dict):
        return any(PROVIDER_CREDENTIAL_PATTERN.search(str(key)) or find_secret(item) for key, item in value.items())
...
def evaluate(campaign_path: Path, fixture: Path | None = None) -> dict[str, object]:
    ...
    return {
        ...
        "assets": assets,
        "isolation": {
            "network_namespace": "active",
            "provider_credentials_present": provider_credentials,
            "python_audit_hook": "active",
        },
        "phase_0b": campaign["phase_0b"]["status"],
    }


def run_preflight(fixture_value: str | None = None) -> int:
    repo_root = Path(os.environ["REPO_ROOT"])
    protocol_dir = repo_root / ".context/benchmarks/model-roi/task-parallelism"
    fixture = Path(fixture_value).resolve() if fixture_value else None
    campaign_path = fixture if fixture and fixture.name != "invalid-asset-manifest.json" else protocol_dir / "campaign.phase-0a.json"
    try:
        report = evaluate(campaign_path, fixture)
...
def main() -> int:
    ...
    if not args.validate_only:
        parser.error("only --validate-only is supported outside the isolated launcher")
