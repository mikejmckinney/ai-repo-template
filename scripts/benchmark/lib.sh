...
apply_orchestration_variant() {
  ...
  case "${platform}" in
    copilot)
      overlay_dir="${wt}/.github/agents"
      suffix=".agent.md"
      ;;
    claude-code)
      overlay_dir="${wt}/.claude/agents"
      suffix=".md"
      ;;
    cursor)
      overlay_dir="${wt}/.cursor/agents"
      suffix=".md"
      ;;
    codex)
      overlay_dir="${wt}/.codex/agents"
      suffix=".toml"
      ;;
    gemini-cli)
...
make_worktree() {
  ...
  local wt
  wt="$(worktree_path "${task}" "${alias}" "${ri}")"
  local br
  br="$(branch_name "${task}" "${alias}")-r${ri}"
