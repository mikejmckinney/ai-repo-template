...
| `.github/agents/**` / `.claude/agents/**` / `.cursor/agents/**` / `.codex/agents/**` | Architect | Role definitions; changes require an ADR, must update all platform overlays in lockstep, and `scripts/checks/050-agent-mirror.sh` enforces `description:` parity across the canonical `.agents/**` files and every platform overlay. See `docs/decisions/adr-003-claude-code-subagent-registration.md` and issue #330. |
