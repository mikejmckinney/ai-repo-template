...
#
# Inlined from scripts/test-parallelism-report-parser.sh by issue #280 (un-wrap legacy delegate).
...
_legacy_body() {
  ...
  # ===== inlined body of scripts/test-parallelism-report-parser.sh (issue #280) =====
...
parse_ownership() {
  "$SCRIPT_DIR/parse-ownership-table.sh"
}
...
if [[ -f "$LIVE_OWNERSHIP" ]]; then
  ...
  parser_roles=$("$SCRIPT_DIR/parse-ownership-table.sh" --list-roles | sort -u)
  table_roles=$(awk -F'|' '
    /^\| *[A-Z][A-Za-z]* +\|/ {
      role=$2
...
@test "parallelism-report-parser: inlined test-parallelism-report-parser.sh body passes" {
  run _legacy_body
