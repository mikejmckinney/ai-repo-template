...
#
# Inlined from scripts/test-coordination-sync.sh by issue #280 (un-wrap legacy delegate).
...
_legacy_body() {
  ...
  # ===== inlined body of scripts/test-coordination-sync.sh (issue #280) =====
...
REAL_COORD="$(cd "$SCRIPT_DIR/.." && pwd)/.context/state/coordination.md"
...
@test "coordination-sync: inlined test-coordination-sync.sh body passes" {
  run _legacy_body
  if [ "$status" -ne 0 ]; then
    printf 'STATUS=%s\nOUTPUT:\n%s\n' "$status" "$output" >&2
  fi
