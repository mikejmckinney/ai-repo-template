...
3. Globs `scripts/checks/[0-9]*.sh` in lexical order and `source`s each
   module. Each module performs its checks and increments the shared
   counters via `pass`/`fail`/`warn`.
...
Files use a numeric `<NN>-<concern>.sh` prefix. The orchestrator hard-fails
if any `*.sh` file in `scripts/checks/` lacks a numeric `<NN>-` prefix
(catches typos that would otherwise silently skip a module).

Numbers leave room for insertion: increments of 5 (10-, 15-, 20-, 25-, …)
let new modules slot between existing ones without a renumber.
...
2. Create `scripts/checks/<NN>-<concern>.sh`. Start it with the standard
   header used by the existing modules.
