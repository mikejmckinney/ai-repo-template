...
5. Any `.context/rules/*.md` or `.context/vision/*.md` whose domain covers the files you are about to edit — e.g., read `process_doc_maintenance.md` before changes that may trigger doc-sync requirements; read `domain_code_quality.md` before non-trivial code refactors; read the relevant architecture or mockup in `.context/vision/` before implementing UI or structural changes.
