...
3. `.context/state/_active.md` and the active `.context/state/task_<slug>.md` (if any) — catches staleness in your own working state. `_active.md` is capped at ~20 lines of task metadata (Active Task, File, Role, Blockers, and Next Steps); granular progress, files, and detailed blockers live in the task file it references.
