...
- **Resolve from the repo first.** Before asking, check whether the answer is already in the repo using the Truth hierarchy (`.context/**` → `docs/**` → codebase): config files, `AI_REPO_GUIDE.md`, `.context/rules/`, ADRs, prior task files, or the code itself. If you can resolve the ambiguity by reading, do that instead of asking.
