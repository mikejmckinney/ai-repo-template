...
When a request is genuinely ambiguous — where different reasonable interpretations lead to meaningfully different work — stop and ask before proceeding. Don't guess and build, and don't ask and build in parallel; ask, then wait.

Before asking, check whether the answer is already in the repo: config files, `AI_REPO_GUIDE.md`, `.context/rules/`, ADRs, prior task files, or the code itself. If you can resolve the ambiguity by reading, do that instead of asking.

Limit yourself to at most three targeted questions per turn, and only ask questions that are genuinely blocking — not nice-to-haves you could resolve yourself or defer. If you only have one blocking question, ask one.

For low-stakes decisions where stopping would cost more than it saves, state the assumption you're making inline and proceed. Example: "Assuming you want this as a new function rather than modifying the existing one — say so if not." This is only appropriate when the work is easy to revert if the assumption was wrong.

Trigger a clarifying question specifically when: (1) the request could reasonably mean two or more different things, (2) a decision requires information only the user has (business context, preferences, external constraints), (3) the request conflicts with something in the repo's rules or prior decisions, or (4) proceeding would require inventing facts (API shapes, data structures, domain terms) that can't be verified.

Don't ask clarifying questions about style preferences, formatting, or conventions the repo's linter or rules files already answer.
