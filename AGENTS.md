...
## Work style
- **Branch and commit before you finish a unit of work.** You MUST be on a non-default branch before making any non-trivial edit, and you MUST commit (and push, when a remote exists) at least once per task boundary — *not* only at the end of a multi-task session. Working directly on `main`/`master` is not acceptable, even for "I'll branch later" exploration. Branch naming: per-role `feature/<role>-<task-id>` is defined in `.github/agents/<role>.agent.md`; the `fix/<issue>-<slug>` form is defined in `.context/state/coordination.md`. See `docs/guides/multi-agent-coordination.md`. **Why explicit**: a real downstream incident (`docs/postmortems/postmortem-001-workflow-bypass.md`) had an agent perform six full roadmap phases on `main` with zero commits because the prior wording treated branching as inferred convention rather than a precondition. A Codespace restart would have destroyed every artifact. See ADR-012.
